...
export class StDetailsPage implements OnInit {
  ...
  async Get_Students() {
    ...
    await this.global.GetStudent.subscribe((res) => {
      ...
      this.activeCounter = this.student_details.filter(
        (x: { st_status: string }) => x.st_status === 'active'
      ).length;
      this.deactiveCounter = this.student_details.filter(
        (x: { st_status: string }) => x.st_status === 'deactive'
      ).length;
      this.leaveCounter = this.student_details.filter(
  ...
  async refresh() {
    this.refresher = true;
    setTimeout(() => {
      (this.refresher = false), this.ApiCall.GetStudents(this.data.c_id);
      this.Get_Students();
    }, 1500);
  }
